...
function getRuleName(key: keyof typeof RULE_NAMES, protocol: 'TCP' | 'UDP'): string {
  return key === 'game' ? `${RULE_NAMES.game} ${protocol}` : RULE_NAMES[key]
}

export class FirewallManager {
  private store = new Store<FWStore>({ name: 'firewall', defaults: { customRules: [] } })
  ...
  async getStatus(gamePort: number, rconPort: number, restApiPort: number): Promise<FirewallPort[]> {
    const targets = [
      { name: getRuleName('game', 'UDP'), port: gamePort, protocol: 'UDP' as const },
      { name: getRuleName('game', 'TCP'), port: gamePort, protocol: 'TCP' as const },
  ...
  private async ruleExists(name: string, protocol?: 'TCP' | 'UDP'): Promise<boolean> {
  ...
  private async addRule(
    ...
    try {
      ...
      if (msg.includes('5') || msg.toLowerCase().includes('access')) {
        return { success: false, error: 'Droits administrateur requis. Relancez en tant qu\'administrateur.' }
      }
      return { success: false, error: msg }
    }
  }

  private async deleteRule(name: string, protocol?: 'TCP' | 'UDP'): Promise<void> {
    const protoArg = protocol ? ` protocol=${protocol}` : ''
    try {
      await execAsync(`netsh advfirewall firewall delete rule name="${name}"${protoArg}`, { timeout: 10000 })
    } catch {
  ...
  async disableRule(key: 'game' | 'rcon' | 'restapi'): Promise<{ success: boolean; error?: string }> {
    try {
      if (key === 'game') {
  ...
  async applyAll(
    ...
    for (const [key, port, proto] of rules) {
      ...
      if (!res.success && res.error) errors.push(`${getRuleName(key, proto)}: ${res.error}`)
    }
  ...
  async createCustomRule(
    ...
    if (result.success) {
      ...
      const alreadyStored = existing.some((r) => r.name === name && r.protocol === protocol)
      if (!alreadyStored) {
  ...
  async deleteCustomRule(
    ...
    try {
      ...
      const remaining = this.store.get('customRules').filter(
