...
const api = {
  ...
  config: {
    ...
    setServerPath: (path: string) => ipcRenderer.invoke('config:setServerPath', path)
  },
  // SteamCMD
  steamcmd: {
  ...
  palconfig: {
    ...
    write: (settings: Record<string, unknown>) => ipcRenderer.invoke('palconfig:write', settings)
  },
  // Monitor
  monitor: {
  ...
  firewall: {
    ...
    enableRule: (key: 'game' | 'rcon' | 'restapi', port: number, protocol: 'TCP' | 'UDP') =>
